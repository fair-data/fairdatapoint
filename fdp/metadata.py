...
class FAIRConfigReader(object):
    ...
    def _validateParsedMetadata(self):
        ...
        fdp_id, cat_id, dat_id, dist_id = fdp + sfx, cat + sfx, dat + sfx, \
            dist + sfx
        ...
        for section in section_headers:
            for field in mandatoryFields(section.split('/')[0]):
                ...
                # distribution has two alternatives: access_url or download_url
                if '|' in field:
                    a, b = field.split('|')
                    assert(a in fields or b in fields), \
                        _errorFieldInSectionNotFound(field, section)
                else:
                    assert(field in fields), _errorFieldInSectionNotFound(
                        field, section)

                # resource IDs must be unique
                if field in [fdp_id]:  # , cat_id, dat_id, dist_id]:
                    for resource_id in self.getItems(section, field):
                        assert(resource_id not in uniq_resource_ids), \
                            _errorResourceIdNotUnique(resource_id)
...
class FAIRGraph(object):
    ...
    def setMetadata(self, triple):
        assert(isinstance(triple, tuple) and len(triple)
               == 3), 'Input must be a triple (s, p, o).'
        ...
        if resource == 'catalog':
            ...
            g.add((s, DCTERMS.language, LANG.en))
            g.add((s, DCTERMS.identifier, Literal(
                resource_id, datatype=XSD.string)))
